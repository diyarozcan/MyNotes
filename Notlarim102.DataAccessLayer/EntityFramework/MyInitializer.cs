...
namespace Notlarim102.DataAccessLayer.EntityFramework
{
    public class MyInitializer:CreateDatabaseIfNotExists<NotlarimContext>
    {
        protected override void Seed(NotlarimContext context)
        {
            ...
            NotlarimUser standarUser = new NotlarimUser()
            {
                Name = "ferhat",
                Surname = "ozcan",
                Email = "fdzcnsn@hotmail.com",
                ActivateGuid = Guid.NewGuid(),
                IsActive = true,
                IsAdmin = false,
                Username = "ferhatozcan",
                Password = "1234",
                ProfileImageFilename = "user1.jpeg",
            ...
            context.NotlarimUser.Add(admin);
            context.NotlarimUser.Add(standarUser);
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                context.NotlarimUser.Add(user);
            }
            context.SaveChanges();

            //user list for using..

            List<NotlarimUser> userList = context.NotlarimUser.ToList();
