...
namespace Notlarim102.WebApp.Controllers
{
    public class NoteController : Controller
    {
        ...
        {
            var notes = lm.QList().Include("LikedUser").Include("Note").Where(s => s.LikedUser.Id == CurrentSession.User.Id
            ).Select(s => s.Note).Include("Category").Include("Owner").OrderByDescending(x => x.ModifiedOn);
